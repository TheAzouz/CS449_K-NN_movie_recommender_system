...
object Predictor {
  def main(args: Array[String]) {
    ...
    val prediction = computePredictionForKnnWithSpark(train,test,sc,conf_k) //

    //question2(train,test,sc,conf_k)

    //sc.stop()
    //question3(train,test,conf_k,sc)
      ...
      case Some(jsonFile) => {
        var json = "";
        {
          ...
          val answers: Map[String, Any] = Map(
            "Q4.1.1" -> Map(
              "MaeForK=200" -> computeMae(prediction.pred,test)  // Datatype of answer: Double
            ),
            // Both Q4.1.2 and Q4.1.3 should provide measurement only for a single run
            "Q4.1.2" ->  Map(
              "DurationInMicrosecForComputingKNN" -> prediction.knnTime  // Datatype of answer: Double
            ),
            "Q4.1.3" ->  Map(
              "DurationInMicrosecForComputingPredictions" -> prediction.predTime // Datatype of answer: Double
            )
